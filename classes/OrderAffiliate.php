...
class OrderAffiliate
{
    ...
    public static function OrderAffiliateDo($options = array()) {
        ...
        if(!empty($distributor)) {
            //vip分销商
            $distributor['user_name'] = $user_info['user_name'];
...
//    	return $order_goods;
    ...
    public static function special_goods_affiliate_do($three_parent_ids, $special_affiliate_money, $options, $store_affiliate_role, $can_affiliate_role)
    {
    	...
    	$percent = 0;
    	$for_team_affiliate_amount = 0;
    	if (!empty($three_parent_ids)) {
    		foreach ($three_parent_ids as $k => $v) {
    			if (!empty($v)) {
    				$user_info = RC_Api::api('user', 'user_info', array('user_id' => $v));
    				if ($user_info['user_rank'] == '0') {
    					$rankinfo = \Ecjia\App\Yuemeihouapi\UserRank::GetUserRankInfo($user_info['rank_points']);
    					$user_rank = empty($rankinfo['rank_id']) ? 0 : $rankinfo['rank_id'];
    				} else {
    					$user_rank = intval($user_info['user_rank']);
    				}
    				if (!empty($can_affiliate_role) && in_array($user_rank, $can_affiliate_role)) {
    					$invite_user = array_search($user_rank, $store_affiliate_role);
    				}
        			//当前角色的等级，及是第几级直属上级；获取最终分成比例
        			$affiliate_special_goods_percent = RC_Loader::load_app_config('affiliate_special_goods_percent', 'yuemeihouapi');
        			if ($k && !empty($v)) {
        				$percent = self::get_percent($k, $invite_user, $affiliate_special_goods_percent);
        			}
        			$affiliate_amount = $special_affiliate_money * ($percent/100);
        			$for_team_affiliate_amount += $affiliate_amount;

        			//分成记录
                    if ($v && $affiliate_amount > 0) {
                        $log = array(
                                'order_id'      => $options['order_id'],
                                'time'          => RC_Time::gmtime(),
                                'user_id'       => $v,
    	...
    	//金天使获取团队分成比例
    	$affiliate_team_percent = ecjia::config('affiliate_team_percent');

    	if ($affiliate_team_percent > 0) {
    		//下单会员的所有上级有没金天使角色，有金天使角色，给金天使角色团队奖励分成
    		//获取下单会员所有上级
    		$parent_id_string = self::get_parent_id($options['user_id']);
    		if (!empty($parent_id_string)) {
    			$parent_ids = self::get_parent_ids_array($parent_id_string);
    			$parent_ids = array_merge($parent_ids, array($options['user_id'])); //包含下单者自己（也可能是金天使角色）
    		}
    		if (!empty($parent_ids)) {
    			foreach ($parent_ids as $row) {
    				$userinfo = RC_Api::api('user', 'user_info', array('user_id' => intval($row)));
    				if ($userinfo['user_rank'] == '0') {
    					$rankinfo = \Ecjia\App\Yuemeihouapi\UserRank::GetUserRankInfo($userinfo['rank_points']);
    					$user_rank = empty($rankinfo['rank_id']) ? 0 : $rankinfo['rank_id'];
    				} else {
    					$user_rank = intval($userinfo['user_rank']);
    				}
    				//上级角色
    				$user_role = array_search($user_rank, $store_affiliate_role);
    				if ($user_role == 'angel_agent') {
    					$amount = sprintf("%.2f", $for_team_affiliate_amount*$affiliate_team_percent/100);
