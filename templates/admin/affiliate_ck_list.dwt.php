...
<ul class="nav nav-pills">
    <li class="{if $filter.status eq 0}active{/if}">
        <a class="data-pjax" href="{RC_Uri::url('#admin_separate/init')}&status=0{if $smarty.get.order_sn}&order_sn={$smarty.get.order_sn}{/if}">{t domain="affiliate"}未分成{/t}
    ...
    <li class="{if $filter.status eq 1}active{/if}">
        <a class="data-pjax" href="{RC_Uri::url('#admin_separate/init')}&status=1{if $smarty.get.order_sn}&order_sn={$smarty.get.order_sn}{/if}" >{t domain="affiliate"}已分成{/t}
            <span class="badge badge-info">{if $type_count.payed}{$type_count.payed}{else}0{/if}</span>
        </a>
    </li>
    <li class="{if $filter.status eq 2}active{/if}">
        <a class="data-pjax" href="{RC_Uri::url('#admin_separate/init')}&status=2{if $smarty.get.order_sn}&order_sn={$smarty.get.order_sn}{/if}" >{t domain="affiliate"}取消分成{/t}
			  	...
			  	<tr align="center">
			  		<td class="hide-edit-area"><a href='{url path="orders/admin/info" args="order_sn={$log.order_sn}"}' target="_blank">{$log.order_sn}</a>
                        <div class="edit-list">
                            ...
                            <a class="toggle_view ecjiafc-red" href='{url path="affiliate/admin_separate/cancel" args="id={$log.order_id}"}'  data-msg='{t domain="affiliate"}您确定要取消分成吗？此操作不能撤销。{/t}' data-pjax-url='{url path="affiliate/admin_separate/init" args="page={$logdb.current_page}"}' data-val="cancel">{t domain="affiliate"}取消{/t}</a>
                            <!-- {elseif $log.is_separate eq 1} -->
                            <a class="toggle_view ecjiafc-red" href='{url path="affiliate/admin_separate/rollback" args="id={$log.log_id}"}'  data-msg='{t domain="affiliate"}您确定要撤销此次分成吗？{/t}' data-pjax-url='{url path="affiliate/admin_separate/init" args="page={$logdb.current_page}"}' data-val="rollback">{t domain="affiliate"}撤销{/t}</a>
                            <!-- {else} -->
                            -
                            <!-- {/if} -->
                        </div>
