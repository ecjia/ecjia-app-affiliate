 ...
 * @author songqianqian
 */
class admin_distributor extends ecjia_admin {
	...
	public function init() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('分销商列表', 'affiliate')));
		$this->assign('ur_here', __('分销商列表', 'affiliate'));

		$data = $this->get_distributor_list();
		$this->assign('data', $data);
		$this->assign('filter', $data['filter']);
		$this->assign('search_action', RC_Uri::url('affiliate/admin_distributor/init'));
		
        return $this->display('distributor_list.dwt');
	}
	...
	private function get_distributor_list() {
		...
		$filter['distributor_type'] = trim($_GET['distributor_type']);
		...
		if (!empty($data)) {
			foreach ($data as $row) {
				...
				if($row['parent_id']) {
					$row['parent_name'] = RC_DB::table('users')->where('user_id', $row['parent_id'])->pluck('user_name');
				}
				$list[] = $row;
			}
		}
